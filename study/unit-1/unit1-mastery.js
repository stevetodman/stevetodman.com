(function(){
  ...
  function vocabDomains(word){var d=DATA[word];return ['definition','synonym'].concat(d.ant.length?['antonym']:[]);}
  // Unit 1 vocabulary is tested with a word bank. Test-ready therefore means one clean,
  // unassisted retrieval for every teacher-assigned clue family, with no unresolved miss.
  // Harder free recall remains useful remediation, but is not the gate that burns practice time.
  function vocabReady(name,word){return vocabDomains(word).every(function(domain){return independentCorrects(name,word,domain)>=1&&!unresolved(name,word,domain);});}
  ...
  function weakestWords(name,n){return BANK.map(function(w){var domains=vocabDomains(w).concat(['spelling']);return {word:w,score:Math.max.apply(null,domains.map(function(d){return weakness(name,w,d);})),vocab:vocabReady(name,w),spelling:spellingReady(name,w)};}).sort(function(a,b){return b.score-a.score;}).slice(0,n||5);}
  ...
  function addPracticeMs(name,ms){var meta=loadMeta(),p=ensureMeta(meta,name);p.practiceMs=(p.practiceMs||0)+Math.max(0,Number(ms)||0);saveMeta(meta);return p.practiceMs;}
