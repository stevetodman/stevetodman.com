(function(){
  ...
  function fmt(ms){var min=Math.max(0,Math.round((Number(ms)||0)/60000));return min?min+' min recorded on this device':'No timed practice recorded on this device yet';}
  ...
  function mockLine(name,type){var m=M.latestMock(name,type);if(!m)return 'No '+type+' mock on this device yet.';return (type==='vocabulary'?'Vocabulary':'Spelling')+' mock: '+m.correct+'/'+m.total+' · '+new Date(m.at).toLocaleDateString();}
