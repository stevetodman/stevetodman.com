...
assert.match(DEPLOYMENT_ORIGIN, /^https:\/\/[a-f0-9]+\.stevetodman-com\.pages\.dev$/, 'Cloudflare deployment URL is missing or invalid');
...
for (const pathname of ['/study/pin-sprint.html', '/study/us-states.html']) {
  ...
  assert.ok(fs.existsSync(expectedPath), `local production build is missing ${expectedPath}`);
  ...
  assert.equal(
    ...
    digest(expected),
    `${pathname} on the custom domain does not match the target commit's dist build`,
  );
  assert.equal(
    digest(deployment.text),
    digest(expected),
    `${pathname} on the immutable Cloudflare deployment URL does not match the target commit's dist build`,
  );
  assert.equal(
    digest(production.text),
    digest(deployment.text),
    `${pathname} differs between stevetodman.com and the exact Cloudflare deployment`,
  );

  const headers = production.response.headers;
  assert.ok(headers.get('cf-ray'), `${pathname} lacks a Cloudflare response marker`);
  assert.match(headers.get('content-type') || '', /text\/html/i, `${pathname} is not HTML`);
}
...
async function newProductionPage(browser) {
  ...
  // Verify the real production UI without reading or changing family cloud data.
  ...
  const failures = collectRuntimeFailures(page, SITE_ORIGIN);
...
try {
  {
    ...
    assert.equal(await page.locator('a[href="us-states.html"]').count(), 1, 'Pin Sprint must link to the full challenge');
    ...
    await page.waitForFunction(
      previousIndex => window.__pinSprintState().currentIndex > previousIndex,
      before.currentIndex,
    );
...
console.log(JSON.stringify({
  ...
  verified: [
    'production bytes match target dist and exact Cloudflare deployment',
    'Cloudflare HTTPS response and assets',
