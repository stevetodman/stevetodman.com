(() => {
  ...
  const base = window.KDMiscEvidence;
  if (!base) throw new Error('Base KD/MIS-C evidence registry failed to load.');

  const freeze = (value) => Object.freeze(value);
  const item = (value) => freeze(value);

  const harahsheh2026 = freeze({
    short: 'Harahsheh et al., Pediatric Cardiology 2026',
    citation: 'Harahsheh AS, Gunsaulus M, Tierney S, et al. Incomplete Kawasaki Disease Versus Non-severe Multisystem Inflammatory Syndrome in Children: Distinguishing Features from Contemporaneous Patients. Pediatr Cardiol. 2026. doi:10.1007/s00246-026-04444-4.',
    population: 'IKDR, January 2020-October 2023, 40 centers in 8 countries: 769 non-severe MIS-C, 372 unconfirmed incomplete KD, and 146 confirmed incomplete KD.',
    design: 'Multicenter observational registry cohort with contemporaneous enrollment. Non-severe MIS-C excluded shock and ICU admission. Confirmed incomplete KD met 2017 AHA criteria after central data review; unconfirmed incomplete KD had a site diagnosis that could not be confirmed from submitted data.',
  });
  ...
  const mainSignals2026 = [
    ...
    item({ title: '2026 target cohort — Neutrophils', direction: 'Higher across incomplete-KD groups', detail: 'Presentation median 6.3 ×10⁹/L (IQR 4.2-9.0) vs 7.2 (4.2-10.1) vs 10.2 (6.8-13.7); P<.001.', whyNoInput: 'No new bedside neutrophil cutoff was established.' }),
    item({ title: '2026 target cohort — Lymphocytes', direction: 'Lower in non-severe MIS-C at group level', detail: 'Presentation median 1.4 ×10⁹/L (IQR 0.8-2.3) vs 2.9 (1.6-5.0) vs 3.5 (1.8-5.4); P<.001.', whyNoInput: 'No new threshold was derived; the existing Fan ALC threshold remains separately source-attributed.' }),
    item({ title: '2026 target cohort — Platelets', direction: 'Lower in non-severe MIS-C at group level', detail: 'Presentation median 201 ×10⁹/L (IQR 146-291) vs 344 (249-429) vs 470 (350-606); P<.001.', whyNoInput: 'No new platelet cutoff was derived; the existing Fan/CDC threshold remains separately attributed.' }),
    ...
    item({ title: '2026 target cohort — Coronary dimensions', direction: 'Confirmed iKD has greater coronary involvement; MIS-C and unconfirmed iKD overlap', detail: 'Maximum coronary Z-score median 1.3 (IQR 0.6-1.8) vs 1.3 (0.7-1.8) vs 2.0 (1.2-3.8); P<.01.', whyNoInput: 'Coronary findings contribute to confirming incomplete KD, so the exact target study is displayed with incorporation-bias caution rather than used as an independent weight.' }),
