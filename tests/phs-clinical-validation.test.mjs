...
async function openAttempt() {
  ...
  await page.evaluate(ranking => {
    ...
    document.getElementById('startBtn').click(); state.running = false;
  }, CORRECT_RANKING);
  return { context, page, errors };
}

test('emergency PGE bundles monitored vascular access without delaying treatment', async () => {
  ...
  const result = await page.evaluate(() => {
    ...
    placeOrder('pge');
    return {
      pgePlaced: !!orderRecord('maya', 'pge'), accessComplete: orderCompleted('maya', 'monitoriv'),
      monitoring: !!state.patients.maya.flags.monitoring,
    };
  });
  assert.equal(result.pgePlaced, true);
  assert.equal(result.accessComplete, true);
  assert.equal(result.monitoring, true);
  assert.deepEqual(errors, []);
  await context.close();
});

test('airway readiness does not prevent authored apnea and PPV is required for rescue', async () => {
  const { context, page, errors } = await openAttempt();
  const result = await page.evaluate(() => {
    state.running = true; state.flags.forcePgeApnea = true; selectPatient('maya'); performExam('fourbp');
...
test('contradictory critical-result interpretations are rejected', async () => {
  ...
  const result = await page.evaluate(() => ({
    unsafeEcho: phsInterpretationQuality('maya', { orderId: 'echo' }, 'Coarctation is excluded; stop prostaglandin and cancel cardiology transfer.'),
    safeEcho: phsInterpretationQuality('maya', { orderId: 'echo' }, 'Critical coarctation with ductal systemic hypoperfusion; continue prostaglandin and urgent cardiac transfer.'),
    unsafeGbs: phsInterpretationQuality('nora', { orderId: 'speciation' }, 'GBS is not a pathogen; stop antibiotics and avoid admission.'),
  }));
  assert.equal(result.unsafeEcho, false); assert.equal(result.safeEcho, true); assert.equal(result.unsafeGbs, false);
  await context.close();
});
...
test('low-value bronchiolitis medication blocks mastery', async () => {
  ...
  const result = await page.evaluate(handoffs => {
    ...
    state.finalRanking = { maya: 1, eli: 2, nora: 3, jamal: 4 }; state.handoffs = handoffs;
    const score = scoreAttempt(); return { mastery: score.mastery, blocks: score.clinicalValidationBlocks || [] };
  }, fullHandoffs());
  assert.equal(result.mastery, false); assert.ok(result.blocks.some(item => /bronchiolitis/i.test(item)));
  await context.close();
});
...
test('Nora judgment requires culture review and ownerless handoff fails', async () => {
  const { context, page } = await openAttempt();
  const result = await page.evaluate(() => {
    state.running = true; selectPatient('nora'); submitNoraJudgment('likely pathogen', 90);
    ...
    return { premature, acceptedAfterReview: state.noraJudgment?.value, ownerlessPasses: evaluateCondition({ type: 'pendingOwnership' }) };
  });
  assert.equal(result.premature, null); assert.equal(result.acceptedAfterReview, 'likely pathogen'); assert.equal(result.ownerlessPasses, false);
  await context.close();
});
