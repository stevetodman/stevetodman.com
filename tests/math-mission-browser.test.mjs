...
test('diagnostic answer persists, feedback renders, scratchwork works, and the session advances', async () => {
  ...
  try {
    ...
    const beforeStroke = await canvasSnapshot(canvas);
    ...
    await page.mouse.up();
    const afterStroke = await canvasSnapshot(canvas);
    ...
    assert.equal(await canvasSnapshot(canvas), beforeStroke, 'Undo should restore the guide beneath the latest stroke');
    await page.locator('#scratch-clear').click();
    assert.equal(await canvasSnapshot(canvas), beforeStroke, 'Clear should leave the underlying guide intact');
