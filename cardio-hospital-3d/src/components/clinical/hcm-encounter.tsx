...
function HistoryStage() {
  ...
  const visibleFacts = hcmCase.history.filter((fact) => !fact.confidential || encounter.confidentialInterviewDone);
  ...
  const clinicalSignal = recognizedRedFlags === hcmCase.redFlagKeys.length
  ...
  return (
    <div className="clinical-stage">
      <div className="clinical-stage-heading">
        <div><p className="eyebrow">History</p><h3>Choose questions that change pretest probability.</h3></div>
        ...
        <div className="history-privacy">
          <div>
            ...
            <span>{encounter.confidentialInterviewDone
              ...
              : "His mother is still in the room. Ask her to step out before confidential adolescent history."}</span>
          </div>
      ...
      <div className="clinical-response" aria-live="polite">
        {latestFact ? latestFact.answer : "Marcus and his mother wait for your first question."}
      </div>
...
function ExamStage() {
  const encounter = useHospitalStore((state) => getActiveEncounter(state.hospital));
  const dispatch = useHospitalStore((state) => state.dispatch);
  const priorSiteAction = encounter?.performedExamActions.filter((action) => action.startsWith("auscultation:") && action !== "auscultation:valsalva").at(-1);
  const [selectedSite, setSelectedSite] = useState<AuscultationSite>((priorSiteAction?.split(":")[1] as AuscultationSite) || "LLSB");
  ...
  const hasDynamicMurmur = performed.includes("auscultation:LLSB") && performed.includes("auscultation:valsalva");
  ...
  if (latestAction === "extras:pmi") response = hcmCase.exam.extras.find((item) => item.toLowerCase().includes("pmi")) ?? hcmCase.exam.extras.join(" ");
  ...
  const selectSite = (site: AuscultationSite) => {
    ...
    updateSite(site);
    dispatch({ type: "EXAM_PERFORMED", encounterId: encounter.encounterId, action: `auscultation:${site}` });
  };
  ...
  const toggleValsalva = (next: boolean) => {
    setValsalva(next);
    updateValsalva(next);
    if (next) dispatch({ type: "EXAM_PERFORMED", encounterId: encounter.encounterId, action: "auscultation:valsalva" });
  };
  ...
  return (
    <div className="clinical-stage">
      <div className="clinical-stage-heading">
        ...
        <strong className="clinical-signal">{hasDynamicMurmur ? "Dynamic outflow murmur identified" : "Focused exam in progress"}</strong>
      </div>
      ...
      <div className="clinical-action-grid exam-actions">
        ...
        <button type="button" className={`clinical-action${performed.includes("extras:pmi") ? " used" : ""}`} onClick={() => dispatch({ type: "EXAM_PERFORMED", encounterId: encounter.encounterId, action: "extras:pmi" })}>Palpate the PMI</button>
      </div>
      <section className="auscultation-workbench" aria-label="Interactive cardiac auscultation">
        <div>
          <p className="eyebrow">Digital stethoscope</p><h4>{selectedSite}</h4>
          <div className="site-grid">
            {hcmCase.exam.auscultation.map((finding) => (
              <button key={finding.site} type="button" className={`site-button${selectedSite === finding.site ? " selected" : ""}${performed.includes(`auscultation:${finding.site}`) ? " used" : ""}`} onClick={() => selectSite(finding.site)}>{finding.site}</button>
            ))}
          </div>
        </div>
        <div className="auscultation-controls">
          <p>{siteFinding?.description}</p>
          <button type="button" className="primary-action" onClick={toggleListening}>{listening ? "Stop listening" : "Begin listening"}</button>
          <label><input type="checkbox" checked={valsalva} onChange={(event) => toggleValsalva(event.target.checked)} />Ask Marcus to perform Valsalva</label>
      ...
      <div className="clinical-stage-footer">
        ...
        <button type="button" className="primary-action" disabled={!canContinue} onClick={() => {
          stopMurmur(); setListening(false);
          dispatch({ type: "ENCOUNTER_STAGE_CHANGED", encounterId: encounter.encounterId, stage: "tests" });
        }}>Choose diagnostic tests</button>
      </div>
    </div>
  );
}
    ...
    <section className="encounter-workspace" aria-label="Marcus Chen clinical encounter">
      <header className="encounter-workspace-header">
        <div>
          ...
          <h2>{hcmCase.patientName} <small>{hcmCase.age} years · {hcmCase.sex}</small></h2>
      ...
      <div className="encounter-progress"><span>{encounter.stage}</span><span>One patient · one persistent encounter</span></div>
