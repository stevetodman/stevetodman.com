...
function canInteract(){return state.started&&!state.ended&&!state.flags.handoffForced&&(state.running||state.mode==='practice');}
function shiftTiming(){const t=state.caseData.timing||{};return{shift:t.shiftSeconds??900,warn:t.handoffWarningSeconds??720,deadline:t.handoffDeadlineSeconds??840};}
function clinicalTimeRemaining(){return Math.max(0,shiftTiming().deadline-state.time);}
...
function checkShiftWindow(){
  if(state.ended||!state.started)return;const{shift,warn,deadline}=shiftTiming();
  if(state.time>=warn&&!state.flags.handoffWarned){state.flags.handoffWarned=true;showUrgent(`${fmtTime(deadline-warn)} of clinical time remain. Begin closing the loop and prepare the end-of-shift handoff.`);addTimeline('Handoff warning issued: clinical time is nearly over.',null,'system');}
  if(state.time>=deadline&&!state.flags.handoffForced){state.flags.handoffForced=true;state.running=false;addTimeline('Clinical time ended. End-of-shift handoff opened; the shift clock is stopped.',null,'system');showUrgent('Clinical time is over. Complete the end-of-shift handoff to finish the attempt.');openEndShift();return;}
  if(state.time>=shift&&!state.flags.handoffForced)autoEnd('The scheduled shift interval ended.');
}
function updateMaya(){
  ...
  else if(p.flags.pgeApnea){
    const apneaAt=p.flags.pgeApneaAt??state.time;p.flags.postTreatment=false;p.flags.responding=false;p.flags.stabilized=false;
    p.vitals.SpO2=Math.max(45,Number(p.vitals.SpO2)-.05);p.vitals.HR=Math.max(42,Number(p.vitals.HR)-.06);p.vitals.Lactate=Math.min(9,Number(p.vitals.Lactate)+.01);
    if(state.time-apneaAt>=150&&!p.flags.arrest){p.flags.arrest=true;p.vitals={...p.vitals,HR:38,SpO2:52,BP:'unobtainable'};addTimeline('Maya arrests from unsupported prostaglandin-associated apnea.','maya','critical');autoEnd('Maya arrested from prostaglandin-associated apnea without airway support.');}
  }
...
function completeSpecial(special){if(special==='nora-speciation'&&!state.flags.noraSpeciated){state.flags.noraSpeciated=true;addResult('nora','speciation','Blood-culture speciation','Streptococcus agalactiae (group B Streptococcus) identified from the blood-culture bottle.','bad',true);addPage('nora','Microbiology: GBS identified','Definitive organism identification is available and requires action.',true,'nora-gbs');}if(special==='pge-apnea'&&!state.flags.pgeApneaResolved){const p=state.patients.maya;if(!p.flags.airwayReady){p.flags.pgeApnea=true;p.flags.pgeApneaAt=state.time;p.flags.critical=true;p.vitals.SpO2=68;p.vitals.HR=88;addTimeline('Maya develops apnea and bradycardia shortly after prostaglandin initiation without airway support immediately ready.','maya','critical');addPage('maya','Bedside emergency: apnea after prostaglandin','Maya is apneic with bradycardia and requires immediate ventilatory support.',true,'maya-apnea');}else addTimeline('Airway support was immediately available during prostaglandin initiation.','maya','safety');}}
