...
test("exam room 3 advances the case graph without a placeholder patient NPC", async () => {
  ...
  assert.match(attending, /GenericMeshYawOffset = -90\.f/);
  assert.match(attending, /AcknowledgeDistanceCm = 700\.f/);
  assert.match(attending, /if \(ToLearner\.Size2D\(\) <= AcknowledgeDistanceCm\)[\s\S]*FaceToward/);
  ...
  assert.match(source, /Hand Hygiene/);
  assert.match(source, /Clinic Directory/);
  ...
  assert.match(await read("Source/CardioHospital/Private/CardioBlockoutNPC.cpp"), /SK_LabCoat/);
