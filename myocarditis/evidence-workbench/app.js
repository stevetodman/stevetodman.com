...
function SourceList({sources,selected,onSelect,filter,setFilter,query,setQuery,overrides}){
  ...
  return h('section',{className:'panel source-panel'},
    ...
    h('div',{className:'toolbar'},
      h('input',{className:'search',placeholder:'Search sources…',value:query,onChange:e=>setQuery(e.target.value)}),
    ...
    h('div',{className:'workspace'},
      ...
      h('section',{className:'panel pdf-panel'},
        h('div',{className:'pdf-toolbar'},
          h('button',{className:'btn primary',onClick:()=>fileRef.current?.click()},pdfBlob?'Replace PDF':'Attach PDF'),h('input',{ref:fileRef,type:'file',accept:'application/pdf',hidden:true,onChange:e=>attachPdf(e.target.files?.[0])}),
